    ...
    Layer.provide(
      Layer.mergeAll(
        ...
        DesktopConfig.layerTest({
          AGENT_PLOT_HOME: baseDir,
          AGENT_PLOT_PORT: "9999",
          AGENT_PLOT_MODE: "desktop",
          AGENT_PLOT_DESKTOP_LAN_HOST: "192.168.1.50",
  ...
  it.effect("resolves backend start config with a stable scoped bootstrap token", () =>
    withHarness(
      Effect.gen(function* () {
        ...
        assert.isUndefined(first.env.AGENT_PLOT_PORT);
        assert.isUndefined(first.env.AGENT_PLOT_MODE);
        assert.isUndefined(first.env.AGENT_PLOT_DESKTOP_LAN_HOST);
        ...
        assert.equal(first.bootstrap.agentPlotHome, environment.baseDir);

...
const defaultEnvironmentInput = {
  ...
  processArch: "arm64",
  appVersion: "1.2.3",
  appPath: "/Applications/Agent Plot.app/Contents/Resources/app.asar",
  isPackaged: true,
  resourcesPath: "/Applications/Agent Plot.app/Contents/Resources",
...
const makeElectronAppLayer = (calls: ElectronAppCalls) =>
  Layer.succeed(ElectronApp.ElectronApp, {
    ...
    name: Effect.succeed("Agent Plot"),
      ...
      DesktopAppIdentity.layer.pipe(
        Layer.provideMerge(
          FileSystem.layerNoop({
            exists: (path) =>
              Effect.succeed(input.legacyPathExists === true && path.includes("Agent Plot (Alpha)")),
            readFileString: () =>
              Effect.succeed(input.packageJson ?? '{"agentPlotCommitHash":"abcdef1234567890"}'),
          }),
        ),
  ...
  it.effect("keeps using the legacy userData path when it already exists", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(userDataPath, "/Users/alice/Library/Application Support/Agent Plot (Alpha)");
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Agent Plot (Alpha)"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Agent Plot (Alpha)");
      ...
      }),
      {
        ...
        environment: {
          env: {
            AGENT_PLOT_COMMIT_HASH: "0123456789abcdef",
          },
        },

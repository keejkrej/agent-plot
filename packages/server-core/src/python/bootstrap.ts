...
function findExtractedUv(dir: string): string | null {
  const candidates = [
    ...
    path.join(dir, process.platform === "win32" ? "uv-x86_64-pc-windows-msvc" : "uv-aarch64-apple-darwin", process.platform === "win32" ? "uv.exe" : "uv"),
    path.join(dir, "uv-x86_64-unknown-linux-gnu", "uv"),
  ];

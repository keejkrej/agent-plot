...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return makeDesktopEnvironmentLayer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ AGENT_PLOT_HOME: baseDir, ...env })),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { AGENT_PLOT_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        AGENT_PLOT_DESKTOP_LAN_HOST: "10.0.0.7",
        AGENT_PLOT_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );

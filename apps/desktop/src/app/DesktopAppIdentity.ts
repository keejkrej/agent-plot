...
const AppPackageMetadata = Schema.Struct({
  agentPlotCommitHash: Schema.optional(Schema.String),
});
...
>()("agent-plot/desktop/AppIdentity") {}
    ...
    return yield* Option.match(raw, {
      ...
      onSome: (value) =>
        decodeAppPackageMetadata(value).pipe(
          Effect.map((parsed) =>
            Option.fromNullishOr(parsed.agentPlotCommitHash).pipe(Option.flatMap(normalizeCommitHash)),
          ),
